...
namespace DuiLib
{
	...
	CCompareValueButtonUI::CCompareValueButtonUI()
	{
		...
		m_dbZero = 0.0;
		m_nDecimal = 0;
		m_enablePaintText = true;
		m_enableCompare = true;
		m_enableFabs = true;
		m_enableSign = false;
		m_dbValueUnit = 1.0;
		m_nColorLeftPadding = 2;
		memset(m_dwCompareColor, 0, sizeof(m_dwCompareColor));
	}

	LPCTSTR CCompareValueButtonUI::GetClass() const
	{
		return _T("CompareValueButtonUI");
	}

	LPVOID CCompareValueButtonUI::GetInterface(LPCTSTR pstrName)
	{
		if (_tcsicmp(pstrName, DUI_CTR_COMPARE_VALUE_BUTTON) == 0) return static_cast<CCompareValueButtonUI*>(this);
		return CButtonUI::GetInterface(pstrName);
	}

	void CCompareValueButtonUI::SetDataValue(double dbValue)
	{
		m_dbValue = dbValue;
	}

	void CCompareValueButtonUI::SetDataZero(double dbValue)
	{
		m_dbZero = dbValue;
	}

	void CCompareValueButtonUI::SetDataUnit(double dbUnit)
	{
		m_dbValueUnit = dbUnit;
	}

	void CCompareValueButtonUI::SetDecimal(int nDecimal)
	{
		m_nDecimal = nDecimal;
	}

	void CCompareValueButtonUI::SetCompareColor(DWORD dwColorUp, DWORD dwColorDown, DWORD dwZero)
	{
		m_dwCompareColor[0] = dwColorUp;
		m_dwCompareColor[1] = dwColorDown;
		m_dwCompareColor[2] = dwZero;
	}

	void CCompareValueButtonUI::EnablePaintText(bool bEnable)
	{
		m_enablePaintText = bEnable;
	}

	void CCompareValueButtonUI::EnableCompare(bool bEnable)
	{
		m_enableCompare = bEnable;
	}

	void CCompareValueButtonUI::EnableFabs(bool bEnable)
	{
		m_enableFabs = bEnable;
	}

	void CCompareValueButtonUI::EnableSign(bool bEnable)
	{
		m_enableSign = bEnable;
	}

	void CCompareValueButtonUI::SetDataSuffix(LPCTSTR lpszText)
	{
		m_strSuffix = lpszText;
	}

	void CCompareValueButtonUI::SetDataPrefix(LPCTSTR lpszPrefix)
	{
		m_strPrefix = lpszPrefix;
	}

	void CCompareValueButtonUI::SetDataFmt(LPCTSTR lpszFmtText)
	{
		m_strDataFmt = lpszFmtText;
	}

	void CCompareValueButtonUI::SetDataColorWidth(int width)
	{
		m_nColorWidth = width;
	}

	void CCompareValueButtonUI::SetDataColorHeight(int height)
	{
		m_nColorHeight = height;
	}

	void CCompareValueButtonUI::SetColorLeftPadding(int nPadding)
	{
		m_nColorLeftPadding = nPadding;
	}

	void CCompareValueButtonUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if (_tcsicmp(pstrName, _T("decimal")) == 0) SetDecimal(_ttoi(pstrValue));
		else if (_tcsicmp(pstrName, _T("painttext")) == 0) EnablePaintText(_tcsicmp(pstrValue, _T("true")) == 0);
		else if (_tcsicmp(pstrName, _T("compare")) == 0) EnableCompare(_tcsicmp(pstrValue, _T("true")) == 0);
		else if (_tcsicmp(pstrName, _T("paintfabs")) == 0) EnableFabs(_tcsicmp(pstrValue, _T("true")) == 0);
		else if (_tcsicmp(pstrName, _T("valuesuffix")) == 0) SetDataSuffix(pstrValue);
		else if (_tcsicmp(pstrName, _T("paintsign")) == 0) EnableSign(_tcsicmp(pstrValue, _T("true")) == 0);
		else if (_tcsicmp(pstrName, _T("zerovalue")) == 0) SetDataZero(_ttof(pstrValue));
		else if (_tcsicmp(pstrName, _T("datavalue")) == 0) SetDataValue(_ttof(pstrValue));
		else if (_tcsicmp(pstrName, _T("valueunit")) == 0) SetDataUnit(_ttof(pstrValue));
		else if (_tcsicmp(pstrName, _T("datacolorwidth")) == 0) SetDataColorWidth(_ttoi(pstrValue));
		else if (_tcsicmp(pstrName, _T("datacolorheight")) == 0) SetDataColorHeight(_ttoi(pstrValue));
		else if (_tcsicmp(pstrName, _T("colorleftpadding")) == 0) SetColorLeftPadding(_ttoi(pstrValue));
		else if (_tcsicmp(pstrName, _T("comparecolorup")) == 0) 
		{
			if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			m_dwCompareColor[0] = _tcstoul(pstrValue, &pstr, 16);
		}
		else if (_tcsicmp(pstrName, _T("comparecolordown")) == 0)
		{
			if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			m_dwCompareColor[1] = _tcstoul(pstrValue, &pstr, 16);
		}
		else if (_tcsicmp(pstrName, _T("comparecolorzero")) == 0)
		{
			if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
			LPTSTR pstr = NULL;
			m_dwCompareColor[2] = _tcstoul(pstrValue, &pstr, 16);
		}
		else if (_tcsicmp(pstrName, _T("valueprefix")) == 0)
		{
			SetDataPrefix(pstrValue);
		}
		else if (_tcsicmp(pstrName, _T("datafmttext")) == 0)
		{
			SetDataFmt(pstrValue);
		}
		else CButtonUI::SetAttribute(pstrName, pstrValue);
	}

	DWORD CCompareValueButtonUI::GetPaintTextColor()
	{
		if (m_dwTextColor == 0) m_dwTextColor = m_pManager->GetDefaultFontColor();
		if (m_dwDisabledTextColor == 0) m_dwDisabledTextColor = m_pManager->GetDefaultDisabledColor();
		DWORD clrColor = IsEnabled() ? m_dwTextColor : m_dwDisabledTextColor;

		if (IsEnabled() && m_enableCompare)
		{
			if (m_dbValue > m_dbZero)
			{
				clrColor = m_dwCompareColor[0];
			}
			else if (m_dbValue < m_dbZero)
			{
				clrColor = m_dwCompareColor[1];
			}
			else
			{
				clrColor = m_dwCompareColor[2];
			}
	...
	CDuiString CCompareValueButtonUI::GetPaintText()
	{
		CDuiString strData;
		if (!m_enablePaintText)
		{
			return strData;
		}

		CDuiString strPre;
		double dbValue = m_dbValue * m_dbValueUnit;
		if (m_enableSign)
		{
			if (m_dbValue > 0.0)
			{
		...
		if (m_enableFabs)
		{
			dbValue = fabs(dbValue);
		}

		strData.Format(_T("%.*lf"), m_nDecimal, dbValue);
		if (m_strDataFmt.IsEmpty())
		{
	...
	void CCompareValueButtonUI::PaintText(HDC hDC)
	{
		RECT m_rcTextPadding = CButtonUI::m_rcTextPadding;
		GetManager()->GetDPIObj()->Scale(&m_rcTextPadding);
		int nLinks = 0;
		...
		DWORD dwTextColor = GetPaintTextColor();

		// color rect 
		if (m_nColorHeight > 0 && m_nColorWidth > 0)
		{
			RECT rcColor = rc;
			rcColor.left = rcColor.right - m_nColorWidth;
			if (rcColor.left < rc.left)
			{
				rcColor.left = rc.left;
			}

			rc.right = rcColor.left - m_nColorLeftPadding;
			if (rc.right < rc.left)
			{
				rc.right = rc.left;
			}

			rcColor.top = (rcColor.top + rcColor.bottom - m_nColorHeight) / 2;
			rcColor.bottom = rcColor.top + m_nColorHeight;
			if (rcColor.top < rc.top)
			{
				rcColor.top = rc.top;
			}

			if (rcColor.bottom > rc.bottom)
			{
				rcColor.bottom = rc.bottom;
			}

			CRenderEngine::DrawColor(hDC, rcColor, GetAdjustColor(dwTextColor));
		}
		
		// text
		CDuiString sText = GetPaintText();
		
		if (m_strDataFmt.IsEmpty() || (m_strPrefix.IsEmpty() && m_strSuffix.IsEmpty()))
		{
			if (!sText.IsEmpty())
			{
		...
		else
		{
			// ¼ÆËãdataµÄ¿í¶È
			RECT rcCalc = { 0,0,0,0 };
			if (sText.GetLength() > m_strDataFmt.GetLength())
			{
				CRenderEngine::DrawText(hDC, m_pManager, rcCalc, sText, m_dwTextColor, m_iFont, DT_CALCRECT | DT_SINGLELINE | DT_LEFT);
			}
			else
			{
				CRenderEngine::DrawText(hDC, m_pManager, rcCalc, m_strDataFmt, m_dwTextColor, m_iFont, DT_CALCRECT | DT_SINGLELINE | DT_LEFT);
			}
			
			int nDataCx = rcCalc.right - rcCalc.left;
			POINT ptMid = {(rc.left+rc.right) / 2, (rc.bottom + rc.top)/2};

			int nPreCX = 0;
			if (!m_strPrefix.IsEmpty())
			{
				CRenderEngine::DrawText(hDC, m_pManager, rcCalc, m_strPrefix, m_dwTextColor, m_iFont, DT_CALCRECT | DT_SINGLELINE | DT_LEFT);
				nPreCX = (rcCalc.right - rcCalc.left);
			}

			int nStufCX = 0;
			if (!m_strSuffix.IsEmpty())
			{
				CRenderEngine::DrawText(hDC, m_pManager, rcCalc, m_strSuffix, m_dwTextColor, m_iFont, DT_CALCRECT | DT_SINGLELINE | DT_LEFT);
				nStufCX = (rcCalc.right - rcCalc.left);
			}

			int nTotal = nDataCx + nPreCX + nPreCX;
			int nX = rc.left;
			if (m_uTextStyle & DT_CENTER)
			{
				nX = ptMid.x - nTotal / 2;
			}
			else if (m_uTextStyle & DT_RIGHT)
			{
				nX = rc.right - nTotal;
			}

			int nVcenter = (m_uTextStyle&DT_VCENTER) ? DT_VCENTER : 0;
			RECT rcDraw = rc;
			rcDraw.left = nX;
			if (nPreCX > 0)
			{
				CRenderEngine::DrawText(hDC, m_pManager, rcDraw, m_strPrefix, m_dwTextColor, m_iFont, DT_SINGLELINE | DT_LEFT | nVcenter);
				rcDraw.left += nPreCX;
			}

			if (!sText.IsEmpty())
			{
				rcDraw.right = rcDraw.left + nDataCx;
				CRenderEngine::DrawText(hDC, m_pManager, rcDraw, sText, dwTextColor, m_iFont, DT_SINGLELINE | DT_RIGHT | nVcenter);
			}
			rcDraw.left += nDataCx;
			...
			if (!m_strSuffix.IsEmpty())
			{
				...
				CRenderEngine::DrawText(hDC, m_pManager, rcDraw, m_strSuffix, m_dwTextColor, m_iFont, DT_SINGLELINE | DT_LEFT | nVcenter);
			}
